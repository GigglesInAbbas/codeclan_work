...
// console.log(biggestNumber(myNumbers));
    ...
    function numberSearch(numbers, searchNnumber){
      for(var number of numbers){
          if(number === searchNnumber){
             return true
          }
      }
      return false
...
for(var item of bill){
  if(item,customer === searchmeal){
  itesm.push(meal)
  }
}
return item
}
// console.log(mealsByCustomerName(myBill))

  // 3.c. Using the functions you wrote in part 3.a and 3.b, find the total that Sally spent.
var SallyMeals = mealsByCustomerName(myBill,"sallys")
var sallysTotalBill = totalBill(SallyMeals)
//  console.log(sallysTotalBill(myBill));
